...
hist(PN, breaks=50, main='14 Year Cost Distribution - Normal', xlab='Final Value')
...
for(i in 1:10000){
  ...
  for(j in 2:14){ # j starts at 2 because we ran the first iteration outside of the loop
    if (j <= 6) {
      r <- rkde(fhat=kde(data$changes), n=1) # using the default for bandwidth
      Pt <- Pt*(1+r) }
    else if (j <= 9) {
      r <- rtri(n=1, min=.07, max=.22, mode=.0917) * -1
      Pt <- Pt*(1+r) }
...
hist(PD, breaks=50, main='14 Year Cost Distribution - Kernel', xlab='Final Value')
abline(v = 1000, col="red", lwd=2)
mtext("Initial Cost", at=1000, col="red")
